...
class IBucketsTest {
	...
	Integer[] KILLMETHO = {whyisthishere, pleasekillme, forty};  
	...
	@Test
	void test() {
		helpTho.bucketize(SHUFFLEDHELL);
		...
		HELPtho.bucketList.get(0).add(one);
		HELPtho.bucketList.get(0).add(two);
		HELPtho.bucketList.get(1).add(helpme);
		HELPtho.dumpBuckets(KILLMETHO);
		assertEquals(KILLMETHO[0].equals(one) , true);
		assertEquals(KILLMETHO[1].equals(two) , true);
		assertEquals(KILLMETHO[2].equals(helpme) , true);
	}
